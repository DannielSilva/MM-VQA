...
def sort_keywords():
    ...
    rows = [{'name':x, 'keys':y} for x, y in zip(keywords_df['name'], keywords_df['keys'])]
    ...
    df = df.sort_values('sorted_keys')
    df.to_csv(os.path.join(KEYWORDS_PATH,'keywords_sorted.csv'), index=False, header=('name','keys','sorted_keys'))
